...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" version="1.0">
  ...
  <xsl:template match="/">
    <xs:text>
    ----------------------------------------------------------------------------------------------------------------------------
	-- NOTE: This script is automatically generated using the 'SchoolYearTypeInserts.xslt' transformation.
    ...
    ----------------------------------------------------------------------------------------------------------------------------
	IF NOT EXISTS(SELECT * FROM edfi.SchoolYearType)
      BEGIN
    </xs:text>
  ...
  <xsl:template match="xs:enumeration">
    ...
    <xsl:text>&#x9;</xsl:text>
    <xsl:text>INSERT INTO [edfi].[</xsl:text><xsl:value-of select="$tableName" />] ([CurrentSchoolYear], [SchoolYearDescription], [SchoolYear]) VALUES (0, '<xsl:call-template name="replace-string">
      <xsl:with-param name="text" select="@value" />
      <xsl:with-param name="replace" select="$apos" />
      <xsl:with-param name="with" select="$apos2" />
    </xsl:call-template>', '<xsl:call-template name="grab-second-year">
      <xsl:with-param name="text" select="@value" />
    </xsl:call-template>');
    <xsl:text />
  </xsl:template>
  <xsl:template name="grab-second-year">
    <xsl:param name="text"/>
