...
function ClientsViewModel() {
    ...
    self.tokenClientClicked = function (client) {
    	...
    	$.support.cors = true;
        $.post(EdFiAdmin.Urls.oauth + "authorize", auth, function (data, err) {
            var token = { Client_id: mydata.Key, Client_secret: mydata.Secret, Code: data.Code || data.code, Grant_type: "authorization_code" };
			...
			.error(function (s, e) {
				alert("Unable to retrieve an authorization code. Please verify that your application key is correct. Alternately, the service address may not be correct: " + EdFiAdmin.Urls.oauth);
			});
    };
