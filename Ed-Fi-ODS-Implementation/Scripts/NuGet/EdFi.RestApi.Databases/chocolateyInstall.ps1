...
try {
    ...
    . $env:chocolateyPackageFolder\DLPDeployment.ps1 $installArgs.environment $installArgs.pathResolverRepositoryOverride $installArgs.certPrefix $installArgs.installType
}
catch {
    Write-ChocolateyFailure "$packageName" "$($_.Exception.Message)"
    throw
