...
namespace GenerateSecurityGraphs
{
    ...
    class Options
    {
        ...
        [Option('s', "server", DefaultValue = "(local)", HelpText = "The name of the SQL Server where the authorization metadata database is located.")]
        public string Server { get; set; }

        [Option('u', "user", HelpText = "The SQL Server username to use for connecting to the authorization metadata database.  Leave username and password blank to use integrated security.")]
        public string User { get; set; }

        [Option('p', "password", HelpText = "The password to use for connecting to the authorization metadata database.  Leave username and password blank to use integrated security.")]
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("Output folder '{0}' does not exist.  Use the --force option to create the directory automatically.", baseFolderPath);
                return;
            }
            ...
            foreach (string claimSetName in claimSetNames)
            {
                ...
                {
                    foreach (string actionName in new[] {"Create", "Read", "Update", "Delete"})
                    {
            ...
            foreach (string claimSetName in claimSetNames.Concat(new[]{String.Empty}))
            {
                ...
                var subgraphs =
                    ...
                    select new
                    {
                        ...
                        UnflattenToDepth = subgraph.Vertices.Count() < 20 ? 0 : (subgraph.Vertices.Count() / 5)
                    })
                    .ToList();
        ...
        private static AdjacencyGraph<Resource, Edge<Resource>> LoadAuthorizationMetadataGraph(string connectionString, out List<string> claimSetNames)
        {
...
from	dbo.ResourceClaims rc
        ...
        left join dbo.ResourceClaimAuthorizationStrategies rcas
...
";
            ...
            var distinctMetadataEdges =
                (from e in metadataEdges
                    select new {e.ResourceName, e.ParentResourceName}
                    ).Distinct();
            ...
            var edgesGroupedByResoureName =
                ...
                into g
                select new { ResourceName = g.Key, Actions = g.Select(x => new { ActionName = x.ActionName, StrategyName = x.AuthorizationStrategyName }) };
                    ...
                    select new
                    {
                        ...
                        Resources =
                            ...
                            select new
                            {
                                ...
                                select new { sg.ActionName, sg.StrategyName })
                                    .ToList()
                            }).ToList()
                    }).ToList();
            ...
            foreach (var claimsetItem in claimsetMetadata)
            {
                ...
                foreach (var resourceItem in claimsetItem.Resources)
                {
                    ...
                    foreach (var actionStrategyItem in resourceItem.ActionStrategy)
                    {
                        ...
                        {
                            var newAction = new ActionAndStrategy { ActionName = actionStrategyItem.ActionName };
        ...
        private static void AddChildren(AdjacencyGraph<Resource, Edge<Resource>> resourceGraph, AdjacencyGraph<Resource, Edge<Resource>> subgraph, Resource vertex)
        {
...
>",
                    resource.Name,
                    GetActionColor(effectiveCreatePermissions), effectiveCreatePermissions.AuthorizationStrategy, GetAuthorizationStrategyText(effectiveCreatePermissions),
                    GetActionColor(effectiveReadPermissions), effectiveReadPermissions.AuthorizationStrategy, GetAuthorizationStrategyText(effectiveReadPermissions),
                    GetActionColor(effectiveUpdatePermissions), effectiveUpdatePermissions.AuthorizationStrategy, GetAuthorizationStrategyText(effectiveUpdatePermissions),
                    GetActionColor(effectiveDeletePermissions), effectiveDeletePermissions.AuthorizationStrategy, GetAuthorizationStrategyText(effectiveDeletePermissions),
                    Colors.Header,
        ...
        {
            return !effectiveActionAndStrategy.ActionInherited ? @"<B>" : string.Empty;
        }
            ...
            Resource resource, string actionName, string claimSetName)
        {
            ...
            var inheritedDefaultActionAndStrategyPairs =
                (from r in resource.Ancestors
                 ...
                 where pair != null
                 select new { OriginatingResourceName = r.Name, ActionAndStrategy = pair})
                    .Reverse()
                    .ToList();
            ...
            var claimsetOverridePermissions =
                (from r in resource.Ancestors
                 ...
                 select new { OriginatingResourceName = r.Name, ActionAndStrategy = claimsetPair})
                 .ToList();
        ...
        private static string GetActionColor(EffectiveActionAndStrategy effectiveActionAndStrategy)
        {
            ...
            {
                case "Create":
                    return effectiveActionAndStrategy.ActionInherited ? SecondaryColors.Create : Colors.Create;
                case "Read":
                    return effectiveActionAndStrategy.ActionInherited ? SecondaryColors.Read : Colors.Read;
                case "Update":
                    return effectiveActionAndStrategy.ActionInherited ? SecondaryColors.Update : Colors.Update;
                case "Delete":
                    return effectiveActionAndStrategy.ActionInherited ? SecondaryColors.Delete : Colors.Delete;
                default:
                    return Colors.Default;
            }
        }
