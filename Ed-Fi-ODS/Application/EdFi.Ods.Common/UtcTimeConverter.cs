...
namespace EdFi.Ods.Common
{
    ...
    {
        private readonly Regex timeParser = new Regex(@"^(?<Hours>[0-9]{2}):(?<Minutes>[0-9]{2})(:(?<Seconds>[0-9]{2}))?((?<UTC>Z)|((?<OffsetHours>[\-\+][0-9]{2})(:(?<OffsetMinutes>[0-9]{2}))?)?)$", RegexOptions.Compiled);
        ...
        public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            ...
            {
                throw new Exception(
                    String.Format("Unexpected token parsing time. Expected string, found '{0}'.",
                        reader.TokenType));
            }
            ...
            int.TryParse(match.Groups["OffsetHours"].Value, out offsetHours);
            int.TryParse(match.Groups["OffsetMinutes"].Value, out offsetMinutes);
