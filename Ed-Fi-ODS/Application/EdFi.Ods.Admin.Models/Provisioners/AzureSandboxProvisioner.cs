    ...
    public class AzureSandboxProvisioner : ISandboxProvisioner
    {
        ...
        public void AddSandbox(string sandboxKey, SandboxType sandboxType)
        {
            switch (sandboxType)
            {
                case SandboxType.Minimal:
                    CopySandbox(DatabaseNameBuilder.TemplateMinimalDatabase, DatabaseNameBuilder.SandboxNameForKey(sandboxKey));
                    break;

                case SandboxType.Sample:
                    CopySandbox(DatabaseNameBuilder.TemplateSampleDatabase, DatabaseNameBuilder.SandboxNameForKey(sandboxKey));
                    break;

                case SandboxType.Empty:
                    CopySandbox(DatabaseNameBuilder.TemplateEmptyDatabase, DatabaseNameBuilder.SandboxNameForKey(sandboxKey));
                    break;
        ...
        public string[] GetSandboxDatabases()
        {
            ...
            var result = ObjectContext.ExecuteStoreQuery<string>(
                "SELECT name FROM sys.databases WHERE name like @name;", parameters).ToArray();
            return result;
        }
        ...
        public static void ResetDemoSandboxWorker(object obj = null)
        {
            ...
            prov.CopySandbox(DatabaseNameBuilder.TemplateSampleDatabase, tmpName);
            ...
            if (result.Code == 0)
            {
                prov.DeleteSandbox(DatabaseNameBuilder.EdFiDatabase);
                prov.RenameSandbox(tmpName, DatabaseNameBuilder.EdFiDatabase);
            }
        }
