...
{
    /// <summary>
    /// Defines an attribute that must be applied to methods containing assertions.
    /// </summary>
    ...
    public abstract class ScenarioFor<T>
    {
        ...
        private List<string> _constructorArgDisplayNames = new List<string>();
        ...
        protected ScenarioFor()
        {
            var constructorDetails =
                (from c in typeof(T).GetConstructors()
                 ...
                 select new {Constructor = c, Parameters = p})
                    .FirstOrDefault();

            _constructor = constructorDetails.Constructor;

            _mocksByType = new Dictionary<Type, Lazy<object>>();
            //var mocksByParameterName = new Dictionary<string, object>();
            ...
            foreach (var parameter in constructorDetails.Parameters)
            {
                ...
                if (parameterType.IsArray)
                {
                    ...
                    _constructorArgs.Add(lazyArg);
                }
                else
                {
                    try
                    {
                        lazyArg = new Lazy<object>(() => MockRepository.GenerateStub(parameterType));
                    ...
                    {
                        throw new Exception(
                            string.Format("Unable to create stub of type '{0}'.", parameterType.Name), ex);
                    }
                }

                _constructorArgTypes.Add(parameterType);
                _mocksByType[parameterType] = lazyArg;

                _constructorArgDisplayNames.Add(string.Format(
                    "{0} ({1})",
                    parameterType.Name,
                    parameter.Name));
            }
            ...
            where TDependency : class
        {
            ...
            var argIndex = _constructorArgs.Select((a, i) => _constructorArgTypes[i] == typeof(TDependency) ? i : -1)
                .Max();
            ...
            where TDependency : class
        {
            ...
            if (!_mocksByType.TryGetValue(typeof(TDependency), out dependency))
            {
                ...
                try { firstFromCollection = The_first<TDependency>(); }
                catch {}
        ...
        private dynamic GetDependencyList<TDependency>() where TDependency : class
        {
            dynamic list;
        ...
        public void RunOnceBeforeAny()
        {
            ...
            {
                if (uninitializedArgDisplayNames.Any())
                    Console.WriteLine("The following dependencies were only stubbed for the test:\r\n    {0}\r\n",
                        string.Join("\r\n    ", uninitializedArgDisplayNames));
            }
        }
