...
Scenario Outline: The Read content type only includes certain properties
    ...
    Then the response model should contain the id and the primary key properties of [schoolId]
    And the response model should contain the explicitly included properties of [nameOfInstitution, operationalStatusType, charterApprovalSchoolYearTypeReference, type, administrativeFundingControlDescriptor, addresses, schoolCategories]
...
Scenario Outline: The Read content type only excludes certain properties
    ...
    When a GET (by id) request is submitted using <call mechanism> to schools with an accept header content type of the appropriate value for the profile in use
    Then the response model should contain the id and the primary key properties of [schoolId]
    And the response model should not contain the explicitly excluded properties of [nameOfInstitution, operationalStatusType, charterApprovalSchoolYearTypeReference, type, administrativeFundingControlDescriptor, addresses, schoolCategories]
