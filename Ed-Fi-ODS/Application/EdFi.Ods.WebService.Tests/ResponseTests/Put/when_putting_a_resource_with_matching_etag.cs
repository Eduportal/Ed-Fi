...
{
    [TestFixture]
    public class when_putting_a_resource_with_matching_etag : OwinTestBase
    {
        private static readonly string _databaseName = string.Format("EdFi_Tests_when_putting_a_resource_with_matching_etag_{0}", Guid.NewGuid().ToString("N"));
            ...
            using (var startup = new OwinStartup(DatabaseName, LocalEducationAgencyIds))
            {
                using (var server = TestServer.Create(startup.Configuration))
                {
                    using (var client = new HttpClient(server.Handler))
                    {
                        ...
                        client.DefaultRequestHeaders.TryAddWithoutValidation("If-Match", getById.Headers.ETag.Tag.Replace("\"", string.Empty));
