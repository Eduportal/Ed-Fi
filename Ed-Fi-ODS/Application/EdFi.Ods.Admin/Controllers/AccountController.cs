...
namespace EdFi.Ods.Admin.Controllers
{
    public class AccountController : Controller
    {
        ...
        public AccountController(IUserAccountManager userAccountManager, IPasswordService passwordService,
                                 ISecurityService securityService, IClientAppRepo clientAppRepo)
        {
        ...
        public ActionResult ResetPassword(string marker)
        {
            ...
            if (userProfile != null)
            {
                return View(new PasswordResetViewModel {Marker = marker, UserName = userProfile.Email});
            }
        ...
        public ActionResult ResetPassword(PasswordResetModel model)
        {
            ...
            {
                return View(new PasswordResetViewModel {Marker = model.Marker, UserName = model.UserName});
            }
        ...
        [HttpGet]
        public ActionResult ActivateAccount(string marker)
        {
            var validation = _passwordService.ConfirmAccount(marker);
            if (validation.Success)
            {
                return View(new PasswordResetViewModel {Marker = marker, UserName = validation.UserName});
            }
        ...
        {
            var result = _userAccountManager.ResendConfirmation(model);
            if (result.Success)
        ...
        public ActionResult PasswordResetSuccessful()
        {
            ...
            return View(new PasswordResetCompletViewModel {Message = (string) message});
        }
        ...
        public JsonResult Login(LoginModel model)
        {
            if (ModelState.IsValid)
            {
                if (WebSecurity.Login(model.EmailAddress, model.Password, persistCookie: model.RememberMe))
                {
                    ...
                    return Json(new {success = true, name = user.FullName, authenticated = true});
                }

                //Failed Login
                Response.StatusCode = (int) HttpStatusCode.Unauthorized;
                Response.SuppressFormsAuthenticationRedirect = true;
                return Json(new {success = false, message = "The email address or password provided is incorrect."});
            }
        ...
        private object BuildJsonResponseFromModelState()
        {
            ...
            var data = new {success = false, message, failingFields};
            return data;
        }
...
//        [ValidateAntiForgeryToken]
        ...
        {
            WebSecurity.Logout();
            return Json(new {success = true, authenticated = false});
        }

        private string[] GetFailedFieldsFromModelState()
        {
            return ModelState.Where(x => x.Value.Errors.Count > 0).Select(x => x.Key).ToArray();
        }
