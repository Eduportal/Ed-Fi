...
namespace EdFi.Ods.Common.ExceptionHandling.Translators
{
    public class SqlServerConstraintExceptionTranslator : IExceptionTranslator
    {
        ...
        *  The statement has been terminated.
        ...
        private static Regex expression = new Regex(@"^The (?<StatementType>INSERT|UPDATE|DELETE) statement conflicted with the (?<ConstraintType>FOREIGN KEY|REFERENCE) constraint ""(?<ConstraintName>\w+)"".*?table ""[a-z]+\.(?<TableName>\w+)""(?:, column '(?<ColumnName>\w+)')?");
