...
namespace EdFi.Ods.Api.Common.Filters
{
    public class ProfilesAuthorizationFilter : IAuthorizationFilter
    {
        ...
        public ProfilesAuthorizationFilter(IApiKeyContextProvider apiKeyContextProvider, IProfileResourcesProvider profileResourcesProvider)
        {
        ...
        public Task<HttpResponseMessage> ExecuteAuthorizationFilterAsync(HttpActionContext actionContext, CancellationToken cancellationToken, Func<Task<HttpResponseMessage>> continuation)
        {
            ...
            if (apiKeyContext != null)
                assignedProfiles = _apiKeyContextProvider.GetApiKeyContext().Profiles.ToList();
                ...
                && IsResourceCoveredByAssignedProfile(assignedProfiles, resourceItemName))
            {
                if (actionContext.Request.Method == HttpMethod.Get)
                    return
                        Task.FromResult(ForbiddenHttpResponseMessage(
                            actionContext,
                            string.Format(
                                "One of the following profile-specific content types is required when requesting this resource: '{0}'.",
                                string.Join("', '", assignedProfiles.Select(p => ProfilesContentTypeHelper.CreateContentType(resourceCollectionName, p, ContentTypeUsage.Readable))))));
                ...
                return
                    Task.FromResult(ForbiddenHttpResponseMessage(
                        ...
                        string.Format(
                            ...
                            string.Join("', '", assignedProfiles.Select(p => ProfilesContentTypeHelper.CreateContentType(resourceCollectionName, p, ContentTypeUsage.Writable))))));
            }
            ...
            if (!IsResourceCoveredByAnotherAssignedProfile(assignedProfiles, resourceItemName, profileContentTypeDetails.Profile))
            {
                ...
                if (actionContext.Request.Method == HttpMethod.Get)
                    return Task.FromResult(ForbiddenHttpResponseMessage(actionContext, string.Format(
                            "One of the following profile-specific content types is required when requesting this resource: '{0}'.",
                            string.Join("', '",
                ...
                if (actionContext.Request.Method == HttpMethod.Put || actionContext.Request.Method == HttpMethod.Post)
                    return Task.FromResult(ForbiddenHttpResponseMessage(actionContext,
                        String.Format(
                            "Based on the assigned profiles, one of the following profile-specific content types is required when updating this resource: '{0}'.",
                            string.Join("', '",
                                GetApplicableContentTypes(
        ...
        private bool IsResourceCoveredByAnotherAssignedProfile(List<string> assignedProfiles, string resourceItemName, string profileContentTypeProfileName)
        {
            return
                assignedProfiles.Intersect(_profileResourcesProvider.GetProfileResources()
                    .Where(x => x.Resource.EqualsIgnoreCase(resourceItemName)
                                && x.ProfileName.EqualsIgnoreCase(profileContentTypeProfileName))
                    .Select(x => x.ProfileName)).Any();
        }
        ...
        private bool IsResourceCoveredByAssignedProfile(IList<string> assignedProfiles, string resourceItemName)
        {
            ...
            return assignedProfiles.Intersect(_profileResourcesProvider.GetProfileResources()
                    .Where(x => x.Resource.EqualsIgnoreCase(resourceItemName)).Select(z => z.ProfileName)).Any();
        }
        ...
        private bool AnyAssignedProfilesCoverTheResource(List<string> assignedProfiles, string resourceItemName)
        {
            ...
            return assignedProfiles
                    .Intersect(_profileResourcesProvider
                        .GetProfileResources()
                        .Where(x => x.Resource.EqualsIgnoreCase(resourceItemName))
                        .Select(x => x.ProfileName)).Any();
        }
        ...
        private bool IsTheResourceInTheProfile(string resourceItemName, string profileContentTypeProfile)
        {
            ...
            return _profileResourcesProvider.GetProfileResources()
                        .Any(x => x.ProfileName.EqualsIgnoreCase(profileContentTypeProfile)
                                 && x.Resource.EqualsIgnoreCase(resourceItemName));
        }

        // Generates a list of assigned profiles that can be used by the client for the sent resource 
        private IEnumerable<string> GetApplicableContentTypes(IEnumerable<string> assignedProfiles, string resourceCollectionName, string resourceItemName, HttpMethod httpMethod)
        {
            ContentTypeUsage contentTypeUsage = httpMethod == HttpMethod.Get ? ContentTypeUsage.Readable : ContentTypeUsage.Writable;
            ...
            return assignedContentTypes
                    .Intersect(
                        ...
                        .Where(x => x.Resource.EqualsIgnoreCase(resourceItemName))
                        .Select(x => ProfilesContentTypeHelper.CreateContentType(
                            CompositeTermInflector.MakePlural(x.Resource), x.ProfileName, contentTypeUsage)))
                        .ToList();
        }
        ...
        private bool TryGetProfileContentType(HttpActionContext actionContext, out string profileContentType)
        {
            ...
            if (actionContext.Request.Method == HttpMethod.Get)
                if (actionContext.Request.Headers.Accept.Any(x => x.MediaType.StartsWith("application/vnd.ed-fi")))
                    profileContentType = actionContext.Request.Headers.Accept.First(x => x.MediaType.StartsWith("application/vnd.ed-fi")).ToString();

            //if the method is put or post then retrieve the contenttype from the contenttype header if it is a profile type
            if (actionContext.Request.Method == HttpMethod.Put || actionContext.Request.Method == HttpMethod.Post)
                if (actionContext.Request.Content.Headers.ContentType.ToString().StartsWith("application/vnd.ed-fi"))
