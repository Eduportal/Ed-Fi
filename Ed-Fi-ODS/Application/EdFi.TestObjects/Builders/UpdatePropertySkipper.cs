    ...
    public class UpdatePropertySkipper : IValueBuilder
    {
        ...
        public ValueBuildResult TryBuild(BuildContext buildContext)
        {
            ...
            if (propertyInfo == null)
                throw new Exception(
                    string.Format("Unable to find property '{0}' on type '{1}' (or its implementations).", 
                    propertyName, buildContext.ContainingType));

            var attributes = this.Factory.CustomAttributeProvider.GetCustomAttributes(propertyInfo, false);

            var attributeTypes = attributes.Select(a => a.GetType());

            if (attributeTypes.Intersect(_ignoreAttributeMarkers).Any())
        ...
        public static void IgnorePropertiesMarkedWith<TAttribute>() where TAttribute : Attribute
        {
