...
namespace EdFi.TestObjects.Builders
{
    public class IgnoreAttributeSkipper : IValueBuilder
    {
        public ValueBuildResult TryBuild(BuildContext buildContext)
        {
            ...
            var ignoredByAttribute = this.Factory.CustomAttributeProvider
                .GetCustomAttributes(propertyInfo, true)
                .Any(a => a.GetType().Name.Contains("Ignore"));

            return ignoredByAttribute ? ValueBuildResult.Skip(buildContext.LogicalPropertyPath) : ValueBuildResult.NotHandled;
        }
