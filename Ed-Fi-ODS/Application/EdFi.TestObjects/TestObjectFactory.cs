...
namespace EdFi.TestObjects
{
    ...
    public interface ITestObjectFactory
    {
        ...
        T Create<T>();
        object Create(string logicalPropertyPath, Type targetType, IDictionary<string, object> propertyValueConstraints, LinkedList<object> objectGraphLineage);
    ...
    {
        private ILog _logger = LogManager.GetLogger(typeof(TestObjectFactory));

        private readonly IValueBuilder[] builders;
        private readonly IActivator activator;
        private readonly ICustomAttributeProvider customAttributeProvider;

        public TestObjectFactory() : this(new BuilderFactory().GetBuilders(), new SystemActivator(), new CustomAttributeProvider())
        {
        }
        ...
        {
            get { return this.customAttributeProvider; }
        }

        public T Create<T>()
        {
            return (T) this.Create(string.Empty, typeof(T), null, null);
        }
        ...
        {
            this.NotSoRandomUniqueId = notSoRandomUniqueIds;
            return this.Create(string.Empty, objectType, null, null);
        }

        public CancellationToken CancellationToken;

        public object Create(string logicalPropertyPath, Type targetType, IDictionary<string, object> propertyValueConstraints, LinkedList<object> objectGraphLineage)
        {
            ...
            if (CancellationToken.IsCancellationRequested)
            {
                ...
                throw new OperationCanceledException(string.Format("Cancellation requested prior to creating value for '{0}' (of type '{1}').", logicalPropertyPath, targetType.Name));
            }
            ...
            var buildContext = new BuildContext(logicalPropertyPath, targetType, propertyValueConstraints,
                    null, objectGraphLineage, BuildMode.Create);

            return this.BuildValue(new List<ValueBuildResult>(), buildContext, customObjectBuildContext).Value;
        }

        private ValueBuildResult BuildValue(List<ValueBuildResult> buildResults, BuildContext buildContext, CustomObjectBuildContext customObjectBuildContext)
        {
            string logicalPropertyPath = buildContext.LogicalPropertyPath;
            Type targetType = buildContext.TargetType;

            _logger.DebugFormat("Initiating build of value for property path '{0}' of type '{1}' (with build context {2})...",
                string.IsNullOrEmpty(logicalPropertyPath) ? "[empty]" : logicalPropertyPath, targetType.Name, buildContext);
            ...
            foreach (var builder in this.builders)
            {
                ...
                if (buildResult.Handled)
                {
                    if (buildResult.ShouldSetValue)
                        _logger.DebugFormat("Value built for property path '{0}' was '{1}'...",
                            string.IsNullOrEmpty(logicalPropertyPath) ? "[empty]" : logicalPropertyPath, buildResult.Value);
            ...
            var customBuildResult = this.TryBuildCustomObject(buildResults, buildContext, customObjectBuildContext);
            
            if (customBuildResult.Handled)
                return customBuildResult;

            throw new Exception(
                string.Format("Unable to create object of type '{0}'{1}.  Consider adding an IValueBuilder implementation to handle creating this type for serialization testing.",
                              targetType.FullName, string.IsNullOrEmpty(logicalPropertyPath) ? string.Empty : "at logical path " + logicalPropertyPath));
        }
        ...
        private class CustomObjectBuildContext
        {
            ...
            public Dictionary<Type, int> CustomObjectDepthByType { get; private set; }
            public Dictionary<Type, int> CustomObjectCollectionSequence { get; private set; }
        }

        private ValueBuildResult TryBuildCustomObject(List<ValueBuildResult> buildResults, BuildContext buildContext, CustomObjectBuildContext customObjectBuildContext)
        {
            ...
            if (depth == this.MaximumHierarchyDepth)
            ...
            { 
                ...
                try
                {
                    object instance = this.activator.CreateInstance(targetType);

                    objectGraphLineage.AddFirst(instance);

                    try
                    {
                        this.BuildAndAssignPropertyValues(logicalPropertyPath, objectGraphLineage,
                            targetType, propertyValueConstraints, buildResults, buildMode, customObjectBuildContext);
                    }
                    finally
                    {
        ...
        public ValueBuildResult[] RandomizeValue(object value, Type explicitValueType, string[] notSoRandomUniqueIds = null)
        {
            ...
            this.NotSoRandomUniqueId = notSoRandomUniqueIds;
            var customObjectBuildContext = new CustomObjectBuildContext();
            this.BuildAndAssignPropertyValues(string.Empty, new LinkedList<object>(new [] {value}), explicitValueType, new Dictionary<string, object>(StringComparer.InvariantCultureIgnoreCase), buildResults, BuildMode.Modify, customObjectBuildContext);

            return buildResults.OrderBy(x => x.LogicalPath).ToArray();
        }

        private void BuildAndAssignPropertyValues(string logicalPropertyPath, LinkedList<object> objectGraphLineage, Type explicitType, IDictionary<string, object> propertyValueConstraints, List<ValueBuildResult> results, BuildMode mode, CustomObjectBuildContext customObjectBuildContext)
        {
            ...
            _logger.DebugFormat("Processing custom object properties in the following order: {0}",
            ...
            while (writableProperties.Count > 0)
            {
                ...
                foreach (var property in writableProperties.ToArray())
                {
                    ...
                    {
                        _logger.DebugFormat("Setting property '{0}' to '{1}' from context.", logicalPropertyPath + "." + property.Name, constraintValue);
                        ...
                        catch (Exception ex)
                        {
                            ...
                            string valueTypeName = buildResult.Value == null ? "N/A" : buildResult.Value.GetType().Name;

                            // This can happen when context is incorrectly used to set a similarly named property and the types are different
                            _logger.DebugFormat("Normal value building will proceed for path '{0}' because the property '{1}' (of type '{2}') could not be set to value '{3}' (of type '{4}') on instance type '{5}' due to the following exception:\r\n{6}.",
                                logicalPropertyPath, property.Name, property.PropertyType.Name,
                                buildResult.Value ?? "[null]", valueTypeName, containingInstance.GetType().Name, ex);
                    ...
                    if (buildResult == null)
                    {
                        ...
                        var buildContext = new BuildContext(logicalPropertyPath + "." + property.Name,
                            property.PropertyType, constraintsToPass, //propertyValueConstraints, 
                            explicitType, objectGraphLineage, mode);

                        buildResult = this.BuildValue(results, buildContext, customObjectBuildContext);
                    ...
                    if (buildResult.ShouldSetValue)
                    {
                        ...
                        {
                            throw new Exception(string.Format("Unable to set property '{0}' to value '{1}' on instance type '{2}'.",
                                property.Name, buildResult.Value, containingInstance.GetType().Name), ex);
                        }
                    }
                ...
                if (writableProperties.Count == lastWritableCount)
                    throw new Exception(string.Format("The attempts to build the following properties have failed to make progress due to 'Defer' responses from the value builders:\r\n{0}.",
                        string.Join(", ", writableProperties.Select(p => p.Name))));
            }
        }
        ...
        private static void SetValue(PropertyInfo property, object containingInstance, object proposedValue)
        {
            ...
            {
                throw new Exception(string.Format("Unable to set property '{0}' to value '{1}' on instance type '{2}'.",
                    property.Name, value, containingInstance.GetType().Name), ex);
            }
        }
        ...
        public string[] GetIgnorePaths(Type t, BuildMode mode)
        {
            ...
            var buildContext = new BuildContext(string.Empty, t, new Dictionary<string, object>(StringComparer.InvariantCultureIgnoreCase), null, null, mode);
            this.BuildValue(valueBuildResults, buildContext, customObjectBuildContext);
            ...
            return valueBuildResults
                ...
                .Select(x => x.StartsWith(typeNamePrefix) ? x.Substring(typeNamePrefix.Length) : x)
                .ToArray();
        }
        ...
        {
            get { return _canCreateField; }
            set { _canCreateField = value; }
        }

        private Func<Type, bool> _canCreateField = type => true;
    }
}
