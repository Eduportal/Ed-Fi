...
namespace EdFi.TestObjects
{
    ...
    {
        private string _logicalPropertyPath;
        private Type _targetType;
        private Type _rawTargetType;
        private IDictionary<string, object> _propertyValueConstraints;
        private Type _containingType;
        private LinkedList<object> _objectGraphLineage;
        private BuildMode _buildMode;

        public BuildContext(string logicalPropertyPath, Type targetType, IDictionary<string, object> propertyValueConstraints, Type containingType, LinkedList<object> objectGraphLineage, BuildMode buildMode)
        {
            if (targetType == null)
                throw new ArgumentNullException("targetType");

            _logicalPropertyPath = logicalPropertyPath;
            ...
            if (propertyValueConstraints == null)
                _propertyValueConstraints = new BuildContextConstraints(); // new Dictionary<string, object>(StringComparer.InvariantCultureIgnoreCase);
            else
                _propertyValueConstraints = new BuildContextConstraints(propertyValueConstraints); // Dictionary<string, object>(propertyValueConstraints, StringComparer.InvariantCultureIgnoreCase);

            _containingType = containingType;
            _objectGraphLineage = objectGraphLineage ?? new LinkedList<object>();
            _buildMode = buildMode;
        }

        public string LogicalPropertyPath
        {
            get { return _logicalPropertyPath; }
        }
        ...
        /// </summary>
        public Type RawTargetType
        {
            get { return _rawTargetType; }
        }

        public IDictionary<string, object> PropertyValueConstraints
        {
            get { return _propertyValueConstraints; }
        }
        ...
        public LinkedList<object> ObjectGraphLineage
        {
            get { return _objectGraphLineage; }
        }
        ...
        public override string ToString()
        {
            if (PropertyValueConstraints.Count == 0)
                return string.Format("[Context #{0}: Building path '{1}' using no values.]",
                    GetHashCode(), string.IsNullOrEmpty(LogicalPropertyPath) ? "[empty]" : LogicalPropertyPath);

            return string.Format("[Context #{0}: Building path '{1}' using {2}.]",
                GetHashCode(), string.IsNullOrEmpty(LogicalPropertyPath) ? "[empty]" : LogicalPropertyPath, 
    ...
    public class BuildContextConstraints : IDictionary<string, object>
    {
        ...
        public BuildContextConstraints(IDictionary<string, object> constraints)
        {
            ...
            if (constraintsAsDictionary == null)
                throw new ArgumentException(string.Format("Constraints supplied to the build context must be backed by a Dictionary<string, object> instance (an instance of '{0}' was passed instead).", constraints.GetType()));

            if (constraintsAsDictionary.Comparer.GetType() != StringComparer.InvariantCultureIgnoreCase.GetType())
                throw new ArgumentException("Constraints supplied to the build context must be backed by a Dictionary<string, object> instance, and must use the StringComparer.InvariantCultureIgnoreCase comparer in order to ensure correct behavior.");
            ...
            set
            {
                ...
                if (_constraints.TryGetValue(key, out existingValue))
                {
                    if (!value.Equals(existingValue))
                        _log.DebugFormat("Overwriting value for '{0}' from '{1}' to '{2}'.",
                            key, existingValue, value);
                    else
                        _log.DebugFormat("Overwriting value for '{0}' with same value ('{1}').",
                            key, value);
                }
...
#pragma warning restore 659
        {
            ...
            return this.AsEnumerable().All(x =>
            {
                object value;
